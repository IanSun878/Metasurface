...
run_time = 2e-12
source_gap = -0.001  # gap distance between the source and the top oxide
...
source_x = 5  # source position in x direction
...
theta = np.deg2rad(theta_f)  # fiber tilt angle
...
box = td.Structure(
    geometry=td.Box.from_bounds(rmin=(-inf_eff, -inf_eff, -inf_eff -t_separation-t_box), rmax=(inf_eff, inf_eff, -t_separation)),
    medium=mat_box,
)
...
def make_2d_sim(p: float, source_x: float) -> td.Simulation:
    ...
    sim_box = td.Box.from_bounds(
        rmin=(-buffer - wg_l, 0, -t_box - t_separation - buffer),
