...
def run_2d_gc_paper(posX):
    # Grating coupler 參數

    h_clad = 0.75  # Cladding layer thickness (um).
    ...
    gaussian_source = td.GaussianBeam(
        ...
        center = [-((wg_l + P_GC * n_p)/2-wg_l) + (src_pos+h_clad)*np.tan(theta_f * np.pi/180) + src_posX, 0, h_clad + h_gc + h_separation + h_GR + h_box + h_sub + src_pos], 
    ...
    mode_monitor = td.ModeMonitor(
        center=[-Lx/2 + 0.5 * wl, 0,h_gc/2 + h_separation + h_GR + h_box + h_sub],
        size=[0,4, 4 * h_gc],
        freqs=freqs,
        mode_spec=mode_spec,
        name="mode_monitor",
    ...
    sim = td.Simulation(
        ...
        structures=[ Box , Substrate , WG  , GCs , GRs],
    ...
    return wl_range , power_db, ce_3d
